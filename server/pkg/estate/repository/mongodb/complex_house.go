...
type complexHouseRepository struct {
	collection *mongo.Collection
}

func NewComplexHouseRepository(db *mongo.Database) estate_repository.IComplexHouseDataRepository {
	collection := db.Collection("complexHouses")
	...
	return &complexHouseRepository{
		collection: collection,
	}
}

func (r *complexHouseRepository) Search(ctx context.Context, skip *int, limit *int, sort estate_entity.ComplexHouseSort, filter *estate_entity.ComplexHouseFilter) ([]*estate_entity.ComplexHouse, int, error) {
	...
	if total == 0 {
		return []*estate_entity.ComplexHouse{}, 0, nil
	}
	...
	switch sort {
	case estate_entity.ComplexHouseSortTitleAsc:
		opts.SetSort(bson.D{{Key: "title", Value: 1}})
	case estate_entity.ComplexHouseSortTitleDesc:
		opts.SetSort(bson.D{{Key: "title", Value: -1}})
	default:
		opts.SetSort(bson.D{{Key: "title", Value: 1}})
	}
	...
	var entities []*estate_entity.ComplexHouse
	for cursor.Next(ctx) {
		var dao *estate_mongodb_dao.ComplexHouseDAO
...
func (r *complexHouseRepository) GetByID(ctx context.Context, id string) (*estate_entity.ComplexHouse, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
...
func (r *complexHouseRepository) Create(ctx context.Context, input estate_entity.ComplexHouse) (*estate_entity.ComplexHouse, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()

	dao := new(estate_mongodb_dao.ComplexHouseDAO).FromEntity(input)
	dao.ID = primitive.NewObjectID()

	res, err := r.collection.InsertOne(ctx, *dao)
	if err != nil {
		return nil, errs.ErrInsertOneComplexHouseCreate
	}

	query := bson.M{"_id": res.InsertedID}
	if err := r.collection.FindOne(ctx, query).Decode(&dao); err != nil {
		return nil, errs.ErrDecodeComplexHouseCreate
	}

	return dao.ToEntity(), nil
}

func (r *complexHouseRepository) Update(ctx context.Context, id string, input estate_entity.ComplexHouse) (*estate_entity.ComplexHouse, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
	...
	if err != nil {
		return nil, errs.ErrConvertIDComplexHouseUpdate
	}
	...
	if err := doc.Decode(&dao); err != nil {
		return nil, errs.ErrDecodeComplexHouseUpdate
	}

	return dao.ToEntity(), nil
}

func (r *complexHouseRepository) Delete(ctx context.Context, id string) error {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()

	eid, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return errs.ErrConvertIDComplexHouseDelete
	}
	...
	if err := doc.Err(); err != nil {
		return errs.ErrDocComplexHouseDelete
	}

	return nil
}

func (r *complexHouseRepository) BulkDelete(ctx context.Context, ids []string) error {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
	...
	for _, v := range ids {
		...
		if err != nil {
			return errs.ErrConvertIDComplexHouseBulkDelete
		}
	...
	if _, err := r.collection.UpdateMany(ctx, query, bson.M{"$set": bson.M{"isDeleted": true}}); err != nil {
		return errs.ErrUpdateManyComplexHouseBulkDelete
	}
