...
func NewVillageRepository(db *mongo.Database) estate_repository.IVillageDataRepository {
...
func (r *villageRepository) Search(ctx context.Context, skip *int, limit *int, sort estate_entity.VillageSort, filter *estate_entity.VillageFilter) ([]*estate_entity.Village, int, error) {
	...
	if total == 0 {
		return []*estate_entity.Village{}, 0, nil
	}
	...
	switch sort {
	case estate_entity.VillageSortTitleAsc:
		opts.SetSort(bson.D{{Key: "title", Value: 1}})
	case estate_entity.VillageSortTitleDesc:
		opts.SetSort(bson.D{{Key: "title", Value: -1}})
	default:
		opts.SetSort(bson.D{{Key: "title", Value: 1}})
	}
	...
	var entities []*estate_entity.Village
	for cursor.Next(ctx) {
		var dao *estate_mongodb_dao.VillageDAO
...
func (r *villageRepository) GetByID(ctx context.Context, id string) (*estate_entity.Village, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
...
func (r *villageRepository) Create(ctx context.Context, input estate_entity.Village) (*estate_entity.Village, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()

	dao := new(estate_mongodb_dao.VillageDAO).FromEntity(input)

	res, err := r.collection.InsertOne(ctx, *dao)
	if err != nil {
		return nil, errs.ErrInsertOneVillageCreate
	}

	query := bson.M{"_id": res.InsertedID}
	if err := r.collection.FindOne(ctx, query).Decode(&dao); err != nil {
		return nil, errs.ErrDecodeVillageCreate
	}

	return dao.ToEntity(), nil
}

func (r *villageRepository) Update(ctx context.Context, id string, input estate_entity.Village) (*estate_entity.Village, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
	...
	if err != nil {
		return nil, errs.ErrConvertIDVillageUpdate
	}
	...
	if err := doc.Decode(&dao); err != nil {
		return nil, errs.ErrDecodeVillageUpdate
	}
...
func (r *villageRepository) Delete(ctx context.Context, id string) error {
	...
	eid, err := primitive.ObjectIDFromHex(id)
	if err != nil {
		return errs.ErrConvertIDVillageDelete
	}
	...
	if err := doc.Err(); err != nil {
		return errs.ErrDocVillageDelete
	}

	return nil
}

func (r *villageRepository) BulkDelete(ctx context.Context, ids []string) error {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(time.Second*5))
	defer cancel()
	...
	for _, v := range ids {
		...
		if err != nil {
			return errs.ErrConvertIDVillageBulkDelete
		}
	...
	if _, err := r.collection.UpdateMany(ctx, query, bson.M{"$set": bson.M{"isDeleted": true}}); err != nil {
		return errs.ErrUpdateManyVillageBulkDelete
	}
