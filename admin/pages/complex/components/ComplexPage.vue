...
import { ComplexDrawer } from "~/widgets/complexDrawer";
import FilterDrawer from "~/pages/properties/components/internal/FilterDrawer.vue";
import {
...
const { result: data, refetch: refresh } = useComplexQuery({
  id,
});

const complex = computed(() => data.value?.complex);
...
async function onComplexSave(): Promise<void> {
  await refresh();
}

async function onComplexDelete(): Promise<void> {
  await refresh();
}
...
<template>
  <div class="px-6 py-7 flex flex-col grow overflow-auto">
    ...
    >
      ...
      <NuxtLink to="/estate/complexes">Жилые комплексы</NuxtLink>
    ...
    <div class="max-w-[800px]">
      ...
      >
        <div class="text-2xl font-bold">{{ complex?.title }}</div>

        <div class="grow text-gray-500 text-xs">ID: {{ complex?.id }}</div>

        <div class="space-x-2">
          <a
            v-if="
              config.public.WEBSITE_URL &&
              complex?.publicationStatus === PublicationStatusEnum.PUBLISHED
            "
            :href="`${config.public.WEBSITE_URL}/complexes/${id}`"
            target="_blank"
      ...
      <div class="text-2xs mb-1">
        ...
        {{ $dayjs(complex?.createdAt).format("DD.MM.YYYY") }}
      </div>

      <div class="text-2xs mb-5">
        <span class="text-gray-500">Изменено</span>
        {{ $dayjs(complex?.updatedAt).format("DD.MM.YYYY") }}
      </div>

      <div v-if="complex?.address" class="text-xs flex space-x-5 mb-4">
      ...
      <div class="text-xs flex space-x-5 mb-4">
        ...
        <div>{{ complex?.cadastrNumber || "-" }}</div>
      </div>

      <div class="text-xs flex space-x-5 mb-4">
        <div class="text-gray-500 w-30 shrink-0">Фотографии</div>

        <div v-if="complex?.images?.length" class="flex flex-wrap gap-2">
          <div
      ...
      <div class="text-xs flex space-x-5 mb-4">
        ...
        <div v-if="complex?.documents?.length" class="flex flex-col gap-2">
          <a
      ...
      <div class="text-xs flex space-x-5 mb-4">
        ...
        <div v-if="complex?.inCity">В черте города</div>

        <div v-else>
          {{ complex?.cityDistance ? `${complex?.cityDistance} км` : "-" }}
        </div>
      </div>
      ...
      <div class="text-xs flex space-x-5 mb-4">
        ...
        <div
          v-html="complex?.description?.replace(/\n/g, '<br />') || '-'"
        ></div>
      </div>
    </div>

    <div class="text-base mb-4 pt-4 font-bold">
      Объекты в {{ complex?.title }}
    </div>
    ...
    <ComplexDrawer
      v-if="showComplexDrawer"
      :id="id"
      @close="showComplexDrawer = false"
      @save="onComplexSave"
      @delete="onComplexDelete"
    />
