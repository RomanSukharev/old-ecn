...
import { useComplexDrawer } from "../composables/useComplexDrawer";
import {
  type DocumentInput,
  type ImageInput,
  useComplexQuery,
...
const {
  ...
  setErrors,
  setTouched,
} = useComplexDrawer(props);

if (props.id) {
  loading.value = true;
  const { onResult, onError } = useComplexQuery({ id: props.id });

  onResult((res) => {
    const complex = res.data.complex;

    address.value = complex.address;
    cadastrNumber.value = complex.cadastrNumber;
    cityDistance.value = complex.cityDistance;
    description.value = complex.description;
    developerID.value = complex.developer?.id;
    documents.value = complex.documents!;
    images.value = complex.images!;
    inCity.value = complex.inCity;
    internalInfo.value = complex.internalInfo;
    isReady.value = complex.isReady;
    lat.value = complex.lat;
    lon.value = complex.lon;
    projectDeclarations.value = complex.projectDeclarations;
    publicationStatus.value = complex.publicationStatus;
    readinessQuarter.value = complex.readinessQuarter;
    readinessYear.value = complex.readinessYear;
    region.value = complex.region;
    seoText.value = complex.seoText;
    subRegion.value = complex.subRegion;
    title.value = complex.title;
    tourLink.value = complex.tourLink;
    youtubeLink.value = complex.youtubeLink;
  ...
  onError(() => {
    ...
    toast.add({
      summary: "Ошибка",
      detail: "Не удалось загрузить жилой комплекс",
      severity: "error",
      life: 5000,
    });
    emit("close");
...
const onSaveHandler = handleSubmit(async (values) => {
  const { mutate, onDone, onError } = useSaveComplexMutation();

  onDone(() => {
    toast.add({
      summary: "Операция завершена",
      detail: "Жилой комплекс успешно сохранён",
      severity: "success",
      life: 5000,
    });
    emit("save");
  ...
  onError(() => {
    toast.add({
      summary: "Ошибка",
      detail: "Не удалось сохранить жилой комплекс",
      severity: "error",
      life: 5000,
    });
  });
